...
function HREmployeePersonalData(props) {
  ...
  const displayEmployeeData = (start, end) => {
    ...
    for (let i = start; i < end; i++) {
      employee[0][4] === "M"? employee[0][4] = "පුරුෂ" :  employee[0][4] = "ස්ත්‍රී";
      employee[0][5] === "married"? employee[0][5] = "විවාහක" :  employee[0][5] = "අවිවාහක"; 
      data.push(
        <FloatingLabel
  ...
  return (
    <fieldset className="border rounded-lg grid lg:grid-cols-3 p-5 gap-5 m-5">
      <legend className="text-slate-600">{props.data === "personal"? "පුද්ගලික තොරතුරු" : "රැකියාව පිළිබ‌ඳ තොරතුරු"}</legend>
