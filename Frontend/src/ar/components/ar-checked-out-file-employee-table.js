...
{/*{status.name}*/}{file.fileNumber}
          ...
          <Table.Cell>{/*{status.type}*/}{file.fileName}</Table.Cell>
          ...
          <Table.Cell >{/*{status.supervisor ? <span className="text-green-500 uppercase flex gap-5 items-center" > <FaCheck/>  Approved</span> : <span className="text-orange-400 uppercase flex gap-5 items-center"> <FaHourglass/>  Pending</span>} */}{file.subjectName}</Table.Cell>
          ...
          <Table.Cell>
          {/*<Link to="/HR/leaveRequest">*/}<Button onClick={()=>checkInFile(file.id)}>Check In</Button>{/*</Link>*/}
          </Table.Cell>
        </Table.Row>
      ))}
